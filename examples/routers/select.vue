<template>
  <div style="margin: 100px;">
    <md-select v-model="model7" style="width:200px" multiple>
      <md-option-group label="Hot Cities">
        <Option v-for="item in cityList1" :value="item.value" :key="item.value">{{ item.label }}</Option>
      </md-option-group>
      <md-option-group  label="Other Cities">
        <Option v-for="item in cityList2" :value="item.value" :key="item.value">{{ item.label }}</Option>
      </md-option-group>
    </md-select>

    <md-select v-model="model1" filterable style="width:200px" prefix="ios-albums">
      <md-option v-for="item in cityList" :value="item.value" :key="item.value">{{ item.label }}</md-option>
    </md-select>

    <md-select v-model="model6" multiple filterable style="width:200px" prefix="ios-albums">
      <md-option v-for="item in cityList" :value="item.value" :key="item.value">{{ item.label }}</md-option>
    </md-select>

    <MdSelect v-model="model1" filterable style="width:200px" prefix="ios-albums">
      <MdOption v-for="item in cityList" :value="item.value" :key="item.value">{{ item.label }}</MdOption>
    </MdSelect>
    ...
    <Select v-model="model1" style="width:200px">
      <Avatar src="https://dev-file.iviewui.com/userinfoPDvn9gKWYihR24SpgC319vXY8qniCqj4/avatar" slot="prefix"
              size="small"/>
      <Option v-for="item in cityList" :value="item.value" :key="item.value">{{ item.label }}</Option>
    </Select>

    <Select v-model="model10" :max-tag-count="3" :max-tag-placeholder="more" multiple style="width:400px"
            prefix="ios-albums">
      <Option v-for="item in cityList" :value="item.value" :key="item.value">{{ item.label }}</Option>
    </Select>
