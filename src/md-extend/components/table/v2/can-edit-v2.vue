...
<script>
  ...
  // import _ from '../../../libs/date-formate';
              ...
              if (!currentRow.editting) {
                if (this.editIncell) {
                  ...
                  if (item.date) {
                    ...
                    return h('span', formatter(parser(val, item.date.format), item.date.format))
                    // return h('span', new Date(val).format(item.date.format))
                ...
                if (item.date) {
                  ...
                  let date = param.row[param.column.key] || item.date.default;//param.row.key || item.date.default;
